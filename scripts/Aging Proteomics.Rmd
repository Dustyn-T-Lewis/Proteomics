...
- Creates a character vector `required_packages` containing the names of all packages my script uses.

2. `for (pkg in required_packages) { … }`

- Starts a loop that goes through each package name stores in `required_packages.`
...
- dplyr’s `across()` selects the sample columns, then applies base `rowSums()` (ignoring NA) to give each row a total count or abundance score.
