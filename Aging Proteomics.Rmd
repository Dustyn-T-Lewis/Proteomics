...
`if (pkg %in% c("ReactomePA", "clusterProfiler", "org.Hs.eg.db", "enrichplot")) { BiocManager::install(pkg, update = FALSE) }`
...
`sarco <- read_excel(..., sheet = "MASTER_SARCO_data")`
...
`mutate(detection_score = rowSums(across(all_of(sample_cols)), na.rm = TRUE))`
...
`if (status_code(r) == 200) { … } else stop("UniProt query failed")`
...
`tibble(Accession = sapply(j$results, …), Gene_Symbol_API = sapply(j$results, …))`
...
utils (base R): write.csv() for writing data frames to CSV files.
...
Computes the log₂ fold-change for each comparison by taking the ratio of mean abundances.
...
stats (base R): Provides t.test() for performing two-sample t-tests and p.adjust() for multiple testing correction.

tibble: Creates the one-row tibbles of p-values in the do() call.

stringr: (Indirectly) if any string operations are used elsewhere; not directly called in this section but part of previous cleaning steps.
...
Orders the joined data frame by descending log₂ fold-change, so the most upregulated aging-associated genes come first.

`{ set_names(.$log2FC_Pre_vs_Young, .$ENTREZID) }`

Constructs a named numeric vector where each element is the log₂ fold-change and its name is the corresponding Entrez ID, as required by enrichment functions like enrichKEGG().

`enrichKEGG(gene = ..., organism = "hsa", pvalueCutoff = 0.10)`

Runs KEGG pathway enrichment using clusterProfiler, specifying human (“hsa”) and including pathways with p-values ≤ 0.10.

`enrichPathway(..., organism = "human")`
...
base R: Provides loop constructs (for), vector creation (c()), string concatenation (paste0()), list indexing ([[ ]]), and name retrieval (names()).
